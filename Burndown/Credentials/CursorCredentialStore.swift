...
/// Silently returns `notSignedIn` on any read failure — including missing database, missing CLI,
/// or non-macOS platforms — so a non-Cursor user pays no startup cost.
nonisolated enum CursorCredentialStore {
    ...
    static func load() throws(UsageError) -> CursorCredentials {
        ...
        // Any failure to read — missing `sqlite3` on PATH, malformed value, non-macOS — is a not-signed-in
        // condition. The menu bar is supposed to keep working when one provider is absent.
    ...
    ///
    /// Returns whatever subset of the keys resolved to a non-empty string, and an empty dictionary on
    /// any failure (sqlite missing, malformed rows). The state database is large enough that it's
    /// worth reading every key Burndown needs in a single query. Keys are escaped against single
    /// quotes only — they are well-known constants, not user input.
